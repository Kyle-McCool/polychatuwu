...
export function PriceTicker({ prices, className = "" }: { prices: PriceItem[]; className?: string }) {
  ...
  return (
    <div className={`relative flex items-center overflow-hidden border-y border-white/15 bg-black/90 ${className}`}>
      <span
        className="z-10 flex shrink-0 items-center bg-white/[0.08] py-1.5 pl-3 pr-6 font-mono text-[11px] font-extrabold uppercase tracking-[0.16em] text-white"
          ...
          {doubled.map((item, i) => {
            ...
            return (
              <span key={item.symbol + i} className="mx-3.5 inline-flex items-center gap-1.5 font-mono text-[12px]">
                ...
                <span className="tabular-nums text-white/60">${fmt(item.price)}</span>
